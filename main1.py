...
class UserModel(Base):
    ...
    posts: Mapped[list["PostModel"]] = relationship(back_populates="owner", cascade="all, delete-orphan")
...
async def get_current_users(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)) -> UserModel:
...
async def register_user(user_data: UserCreate, db: AsyncSession = Depends(get_db)):
    existing = await db.execute(select(UserModel).where((UserModel.username == user_data.username) | (UserModel.email == user_data.email)))
...
# ADDED: Endpoint to get current user details for the frontend
@app.get("/users/me", response_model=UserResponse)
async def read_users_me(current_user: UserModel = Depends(get_current_users)):
    return current_user
...
async def upload_file(
    ...
    try:
        upload_response = imagekit.files.upload(file=file_bytes, file_name=file_name, folder="/insta_posts")
    ...
    post = PostModel(
        caption=caption, url=str(image_url), file_type=content_type,
        file_name=file_name, file_id=str(file_id), is_public=is_public, user_id=current_user.id
    )
    db.add(post)
    await db.commit()
...
@app.get("/feed")
async def get_feed(limit: int = 10, offset: int = 0, db: AsyncSession = Depends(get_db)):
    # UPDATED: Joined with owner to fetch the username for the feed display
    ...
    # Format custom response so frontend doesn't need additional fetches
    return [{
        "id": p.id,
        "caption": p.caption,
        "url": p.url,
...
async def delete_post_by_id(post_id: int, current_user: UserModel = Depends(get_current_users), db: AsyncSession = Depends(get_db)):
